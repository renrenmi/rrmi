...
// 加载播放器
if (bilidm == 1 && danmuon == 1) { var dp=new yzmplayer({autoplay:autoplay,element:document.getElementById('player'),theme:color,logo:logo,video:{url:vodurl,pic:vodpic,type:'auto',},danmaku:{id:dmid,api:dmapi+'?ac=dm',user:user,addition:[dmapi+'bilibili/?av='+av]}});dp.danmaku.opacity(1);} 
else if (danmuon == 1) {var dp=new yzmplayer({autoplay:autoplay,element:document.getElementById('player'),theme:color,logo:logo,video:{url:vodurl,pic:vodpic,type:'auto',},danmaku:{id:dmid,api:dmapi+'?ac=dm',user:user,}});dp.danmaku.opacity(1);
} else {var dp=new yzmplayer({autoplay:autoplay,element:document.getElementById('player'),theme:color,logo:logo,video:{url:vodurl,pic:vodpic,type:'auto',},});$('body').addClass("danmu-off");}
if(vodid!=''){$("#vodtitle").html(vodid+'  第'+vodsid+'话'); }
// 通用点击
        ...
        success: function(data) {
            if (data.code == 23) {
                ...
                $(danmaku).each(function(index, item) {
                    var dammulist = `<d class="danmuku-list" time="${item[0]}"><li>${formatTime(item[0])}</li><li title="${item[4]}">${item[4]}</li><li title="用户：${item[3]}  IP地址：${item[5]}">${item[6]}</li><li class="report" onclick="report(\'${item[5]}\',\'${dantitle}\',\'${item[3]}\',\'${item[4]}\')">举报</li></d>`
...
setInterval(function(){
    ...
    if(stime>trytimes) {
		dp.video.currentTime = 0;dp.pause();
        layer.confirm(trytime+"分钟试看已结束，请登录继续播放完整视频", {anim: 1,title: '温馨提示',btn: ['登录','注册'],yes: function(index, layero){top.location.href = ym+"/index.php/user/login.html";},btn2: function(index, layero){top.location.href = ym+"/index.php/user/reg.html";}});
		} 
	},1000);
}

setTimeout(function() {$("#link1").show();}, 1 * 500);
setTimeout(function() {
    //$("#link2").fadeIn();
    $("#link2").show();
}, 1 * 1000);
